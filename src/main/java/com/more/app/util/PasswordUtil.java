...
public class PasswordUtil implements Serializable
{
	 ...
	 * 
	 * @param user     User to whome's password we are comparing
	 * @param password The unhashed password we are comparing
	 ...
	 * 
	 * @param password String which is to be hashed
	...
	public static boolean isValid(String password)
	{
		...
		{
			ValidationResult rr = validator.apply(password, null);
			if (rr.isError())
			{
				valid = false;
				break;
	...
	/**
	 * Returns a list of {@link Validator} objects which can be attached to Vaadin
	 * {@link TextField}s. The Validators are built based on the the paswword
	...
	public static List<Validator> getValidators()
	{
		...
		validators.add(new StringLengthValidator("Password is too short", getMinPasswordLength(), 999999));
		...
		if (isSpecialCharacterRequired())
		{
			validators.add(new RegexpValidator(".*[^a-zA-Z0-9].*",
					"The password must contain characters other than " + "letters from A to Z or numbers"));
		}
		return validators;
	}
	...
	private static boolean isLowerCaseRequired()
	{
		...
		//return Boolean.getBoolean(SessionHandler.getSecurityProperties()
		//		.getProperty("authentication.password.validation.lowerCaseRequired"));
		return true;
	}
	...
	{
		//return Boolean.getBoolean(SessionHandler.getSecurityProperties()
				//.getProperty("authentication.password.validation.upperCaseRequired"));
		return true;
	}
	...
	/**
	 * Checks if special characters (anything else than numbers and letters from a
	 * to z) are required to be present in the password.
	 * 
	...
	{
		String minLenghtStr = System.getProperty("authentication.password.validation.length");
		int minLenght = 8;
